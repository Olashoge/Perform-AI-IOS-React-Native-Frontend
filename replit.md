...
- **UI/UX**: Features dynamic theming (Light, Dark, System modes) using `ThemeProvider` and `useColors()` hook for reactive color schemes, defined in `lib/theme.ts`. Color palette is "institutional blue + muted accents": light primary=#2D5BFF (blue), primaryDark=#0B1220, accent=#2E8B57 (green), warning=#D08A1D (amber), error=#B65C5C (muted red), background=#F6F7FB, surface=#FFFFFF, text=#0B1220. Dark mode: primary=#5B8AFF, accent=#4CAF6E, warning=#D9A84E, error=#CF7070, background=#090D14, surface=#111827, text=#E8ECF1. Score colors use dedicated scoreGreen/scoreYellow/scoreRed tokens. Tab bar selection uses primary blue (light) / blue (dark). It uses the Inter font and supports iOS 26 liquid glass tab bar. Week computations are consistently ISO 8601, Monday-based, and UTC-driven across client and server.
