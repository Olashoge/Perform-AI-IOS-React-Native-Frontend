...
export function getDayOfWeek(startDate: string | undefined, dayIndex: number): string {
  if (!startDate) return "";
  try {
    const d = new Date(startDate + "T12:00:00");
    d.setDate(d.getDate() + dayIndex - 1);
...
export default function MealPlanContent({ planId, planData, hideTitle }: MealPlanContentProps) {
  const Colors = useColors();
  const styles = useMemo(() => createStyles(Colors), [Colors]);
  const plan = planData;
  ...
  return (
    <View>
      {!hideTitle && <Text style={styles.planTitle}>{title}</Text>}
