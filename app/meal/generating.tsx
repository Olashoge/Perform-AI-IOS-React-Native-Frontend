...
export default function MealGeneratingScreen() {
  ...
  const { data } = useMealPlanStatus(planId ?? null, enabled);
    ...
    <View style={[styles.container, { paddingTop: topInset + 20 }]}>
      {isFailed ? (
        ...
        <View style={styles.centerContent}>
          ...
          <Text style={styles.subtitle}>
            Our AI is building your personalized 7-day meal plan. This usually takes 1-2 minutes.
          </Text>
          ...
          <View style={styles.stagesContainer}>
            {STAGES.map((stage, idx) => {
              ...
              const iconColor = status === "completed" ? Colors.accent : status === "in_progress" ? Colors.primary : Colors.textSecondary;
              return (
                <View key={stage.key} style={styles.stageRow}>
                  <Ionicons name={iconName} size={24} color={iconColor} />
                  <Text style={styles.stageLabel}>{stage.label}</Text>
                  <Text style={[styles.stageStatus, status === "completed" && { color: Colors.accent }, status === "in_progress" && { color: Colors.primary }]}>
...
const createStyles = (Colors: ThemeColors) => StyleSheet.create({
  ...
  title: { fontSize: 22, fontFamily: "Inter_700Bold", color: Colors.text, textAlign: "center", marginBottom: 12 },
  subtitle: {
